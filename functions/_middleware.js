...
// Ensures dynamic SSR <title> and <meta name="description"> on /watch/* routes
...
let lastFetchTime = 0;
const CACHE_TTL_MS = 15000; // 15 seconds — keeps live status fresh
...
async function getScheduleData(env) {
  ...
  try {
    ...
    console.warn('Functions upstream schedule fetch failed:', err);
  }
  return cachedSchedule;
}
...
export async function onRequest(context) {
  ...
  if (pathname === '/data/schedule.bin' || pathname === '/api/schedule' || pathname === '/api/v1/schedule' || pathname === '/schedule_cache.json') {
    const data = await getScheduleData(context.env);
  ...
  // Handle Static Clean URLs for Policy & Info Pages
  ...
  // Handle /watch/* and watch.html requests
  if (pathname.startsWith('/watch/') || pathname === '/watch' || pathname === '/watch.html') {
    try {
      ...
      const scheduleData = await getScheduleData(context.env);
  ...
  // Handle /play/* and play.html requests
  ...
  // Handle root / index routes — inject schedule data so client needs no fetch
